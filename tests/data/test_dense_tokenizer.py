...
from sane.data.tokenizers import DenseTokenizer
...
def test_detokenize_tokenize_equality():
    ...
    tokenizer = DenseTokenizer(tokensize=32, device="cpu", mode="full_model", reference_statedict=reference_checkpoint)
...
def test_tokenize_shape():
    ...
    # Initialize the tokenizer
    tokenizer = DenseTokenizer(tokensize=32, device="cpu", mode="layer_wise", reference_statedict=checkpoint)
...
def test_rebuild_flatten_equality():
    ...
    # Initialize the tokenizer
    tokenizer = DenseTokenizer(tokensize=32, device="cpu", mode="layer_wise", reference_statedict=reference_checkpoint)
...
def test_flatten_shape():
    ...
    # Check the dimensions of the flattened weights
    assert len(flattened_weights[0]) == 16*8+8  # First layer: 16 inputs, 8 outputs, plus bias
    assert len(flattened_weights[1]) == 8*2+2   # Second layer: 8 inputs, 2 outputs, plus bias
...
def test_tokenize_layer_vs_full_model_compatibility():
    ...
    tokenizer_layer = DenseTokenizer(tokensize=32, device="cpu", mode="layer_wise", reference_statedict=None)
    tokenizer_full = DenseTokenizer(tokensize=32, device="cpu", mode="full_model", reference_statedict=None)
