"""
Code mostly taken / inspired by Andrey Karpathy's NanoGpt implementation:
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        if not self.flash:
            ...
            # generates a lower triangular matrix. Used to mask out 'future' information from attention matrix
            if self.causal:
            ...
            # all three methods should be enabled anyways, but let's make it explicit here
...
class GPTTransformerEncoder(nn.Module):
    def __init__(
        self,
        n_layer: int = 12,
        n_head: int = 12,
