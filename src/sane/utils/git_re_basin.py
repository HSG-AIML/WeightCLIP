    ...
    for axis, p in enumerate(ps.axes_to_perm[k]):
        ...
        if p is not None:
            ...
            if not w.shape[axis] == perm[p].shape[0]:
                # we have a mismatch between the axis we're trying to permute and the permutation map we're given
                logging.debug(
                    f"mismatch between w.shape[axis]: {w.shape[axis]} and permutation map {perm[p]} with {perm[p].shape[0]} entries"
                )
                ...
                if w.shape[axis] % perm[p].shape[0] == 0:
                    ...
                    logging.error(
                        f"Mismatch between w of shape {w.shape} for permutation on axis {axis} with permutation map {perm[p]} could not be resolved."
                    )
...
def zoo_resnet18slim_permutation_spec() -> PermutationSpec:
    """
    Permutation spec for ResNet18Slim architecture.
    
    Architecture (ResNet18Slim @ any width):
        - conv1 + bn1: Conv(3, c1) + BN + Act
        - layer1: 2× BasicBlock(c1, c1) - no shortcut
        - layer2: 2× BasicBlock(c1→c2, c2) - first has shortcut
        - layer3: 2× BasicBlock(c2→c3, c3) - first has shortcut
        - layer4: 2× BasicBlock(c3→c4, c4) - first has shortcut
        - avgpool + dropout + fc
    
    At 0.3× width: c = [19, 38, 76, 153]
    At 1.0× width: c = [64, 128, 256, 512]
    
    BasicBlock structure:
        conv1 -> bn1 -> act -> conv2 -> bn2 -> (+ shortcut) -> act
    
    Note: Due to residual connections, we must be careful about permutation constraints.
    For blocks without shortcut (layer1), both output dims match the residual input.
    For blocks with shortcut (layer2-4), the shortcut conv ensures dimension matching.
    """
    conv = (
        lambda name, p_in, p_out: {
            f"{name}.weight": (p_out, p_in),
        }
    )
    dense = (
    ...
    # Helper for a BasicBlock without shortcut (same in/out channels)
    # Pattern: in->inner->out (where in==out for residual add)
    def easyblock(name, p):
        """BasicBlock where in_channels == out_channels (no shortcut needed)."""
        return {
            **conv(f"{name}.conv1", p, f"P_{name}_inner"),
    ...
    # Helper for a BasicBlock with shortcut (channel change)
    # The shortcut ensures both paths match at output
    def shortcutblock(name, p_in, p_out):
        """BasicBlock where in_channels != out_channels (has shortcut)."""
        return {
            **conv(f"{name}.conv1", p_in, f"P_{name}_inner"),
            **norm(f"{name}.bn1", f"P_{name}_inner"),
            **conv(f"{name}.conv2", f"P_{name}_inner", p_out),
    ...
    return permutation_spec_from_axes_to_perm(
        {
            # Input block: conv1 + bn1
            **conv("conv1", None, "P_0"),
            **norm("bn1", "P_0"),
            
            # layer1: 2 BasicBlocks (c1 -> c1), no shortcut
            **easyblock("layer1.0", "P_0"),
            **easyblock("layer1.1", "P_0"),
            
            # layer2: 2 BasicBlocks (c1 -> c2)
            # First block has shortcut (c1 -> c2)
            **shortcutblock("layer2.0", "P_0", "P_1"),
            # Second block is easy (c2 -> c2)
            **easyblock("layer2.1", "P_1"),
            
            # layer3: 2 BasicBlocks (c2 -> c3)
            # First block has shortcut (c2 -> c3)
            **shortcutblock("layer3.0", "P_1", "P_2"),
            # Second block is easy (c3 -> c3)
            **easyblock("layer3.1", "P_2"),
            
            # layer4: 2 BasicBlocks (c3 -> c4)
            # First block has shortcut (c3 -> c4)
            **shortcutblock("layer4.0", "P_2", "P_3"),
            # Second block is easy (c4 -> c4)
            **easyblock("layer4.1", "P_3"),
            
            # fc: Linear(c4, num_classes) - no permutation on output
            **dense("fc", "P_3", None, bias=True),
        }
    )
