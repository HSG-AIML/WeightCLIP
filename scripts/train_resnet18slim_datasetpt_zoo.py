...
class RawCachedDataset(Dataset):
    def __init__(self, dataset: Dataset | None = None, transform: Any | None = None):
        self.transform = transform
        self.data: list[Any] = []
        ...
        had_transform_attr = hasattr(base_dataset, "transform")
        original_transform = getattr(base_dataset, "transform", None)
        if had_transform_attr:
        ...
        try:
            ...
            for idx in indices:
                ...
                if isinstance(image, np.ndarray):
                    ...
                    self.data.append(np.asarray(image) if hasattr(image, "mode") else image)
                targets.append(int(label))
            self.targets = torch.tensor(targets, dtype=torch.long)
        finally:
            if had_transform_attr:
...
def canonical_dataset_name(dataset_name: str) -> str:
    base_name = dataset_name[:-7] if dataset_name.endswith("_resnet") else dataset_name
    return base_name.replace("_", "-")


def width_mult_tag(width_mult: float) -> str:
    value = format(float(width_mult), "g")
    return value.replace(".", "") + "x"


def arch_name(width_mult: float) -> str:
...
def register_pickle_compat_types() -> None:
    ...
    __main__.RawCachedDataset = RawCachedDataset
...
def create_model(num_classes: int, channels_in: int, width_mult: float, nlin: str, dropout: float, init_type: str) -> nn.Module:
    return ResNet18Slim(channels_in=channels_in, o_dim=num_classes, width_mult=width_mult, nlin=nlin, dropout=dropout, init_type=init_type)


def count_training_batches(dataset: Dataset, batch_size: int) -> int:
    return max(1, math.ceil(len(dataset) / max(1, int(batch_size))))
...
def train_one_epoch(model: nn.Module, trainset: Dataset, batch_size: int, optimizer: optim.Optimizer, scheduler: optim.lr_scheduler.LRScheduler, criterion: nn.Module, device: str, num_workers: int, grad_clip: float, step_scheduler_per_batch: bool) -> tuple[float, float, float]:
    model.train()
    total_loss = 0.0
    correct = 0
    total = 0
...
def load_saved_params(exp_dir: Path) -> dict[str, Any] | None:
    params_path = exp_dir / "params.json"
...
def load_run_config(exp_dir: Path) -> dict[str, Any] | None:
    ...
    if run_config_path.exists():
        try:
            ...
            return None
    return load_saved_params(exp_dir)


def write_run_config(exp_dir: Path, seed: int, lr: float, num_epochs: int, batch_size: int, momentum: float, weight_decay: float, width_mult: float, nlin: str, dropout: float, init_type: str, grad_clip: float, checkpoint_start_epoch: int, scheduler_epochs: int | None) -> None:
    run_config = {"seed": seed, "lr": lr, "num_epochs": num_epochs, "batch_size": batch_size, "momentum": momentum, "weight_decay": weight_decay, "width_mult": width_mult, "nlin": nlin, "dropout": dropout, "init_type": init_type, "grad_clip": grad_clip, "checkpoint_start_epoch": checkpoint_start_epoch, "scheduler_epochs": scheduler_epochs}
    with (exp_dir / "run_config.json").open("w") as handle:
        json.dump(run_config, handle, indent=2)


def build_scheduler(optimizer: optim.Optimizer, lr: float, steps_per_epoch: int, num_epochs: int, scheduler_epochs: int | None, resume_step: int) -> optim.lr_scheduler.LRScheduler:
    effective_scheduler_epochs = int(scheduler_epochs if scheduler_epochs is not None else num_epochs)
    last_epoch = int(resume_step - 1) if resume_step > 0 else -1
    return optim.lr_scheduler.OneCycleLR(optimizer, max_lr=lr, epochs=effective_scheduler_epochs, steps_per_epoch=steps_per_epoch, last_epoch=last_epoch)


def train_single_model(seed: int, dataset_dict: dict[str, Any], dataset_meta: dict[str, Any], output_root: Path, device: str, num_epochs: int, scheduler_epochs: int | None, checkpoint_start_epoch: int, eval_every: int, batch_size: int, 
                       lr: float, momentum: float, weight_decay: float, width_mult: float, num_workers: int, nlin: str, dropout: float, init_type: str, grad_clip: float, save_outputs: bool) -> dict[str, Any]:
    random.seed(seed)
    np.random.seed(seed)
    ...
    model = create_model(num_classes=int(dataset_meta["num_classes"]), channels_in=int(dataset_meta["channels_in"]), width_mult=width_mult, nlin=nlin, dropout=dropout, init_type=init_type).to(device)
    optimizer = optim.SGD(model.parameters(), lr=lr, momentum=momentum, weight_decay=weight_decay)
    criterion = nn.CrossEntropyLoss()
    steps_per_epoch = count_training_batches(dataset_dict["trainset"], batch_size=batch_size)
    ...
    best_test_at_best_val_acc = float("nan")

    if save_outputs:
        exp_dir.mkdir(parents=True, exist_ok=True)
        existing_run_config = load_run_config(exp_dir)
        if existing_run_config is not None and not math.isclose(float(existing_run_config.get("lr", float("nan"))), float(lr), rel_tol=0.0, abs_tol=1e-12):
            raise ValueError(f"existing seed directory has lr={existing_run_config.get('lr')} but current run expects lr={lr}; use a different zoo root or clear the conflicting seed dir")
        if existing_run_config is None:
            write_run_config(exp_dir=exp_dir, seed=seed, lr=lr, num_epochs=num_epochs, batch_size=batch_size, momentum=momentum, weight_decay=weight_decay, width_mult=width_mult, nlin=nlin, dropout=dropout, init_type=init_type, grad_clip=grad_clip, checkpoint_start_epoch=checkpoint_start_epoch, scheduler_epochs=scheduler_epochs)
        ...
        if existing_checkpoints:
            ...
            if not checkpoint_path.exists():
                raise FileNotFoundError(f"Missing model checkpoint for resume: {checkpoint_path}")
    ...
    if save_outputs and start_epoch > 0 and last_checkpoint is not None:
        optimizer_path = last_checkpoint / "optimizer"
        scheduler_path = last_checkpoint / "scheduler"
        if not optimizer_path.exists():
            raise FileNotFoundError(f"Missing optimizer state for resume: {optimizer_path}")
        optimizer.load_state_dict(torch.load(optimizer_path, map_location=device, weights_only=False))

    scheduler = build_scheduler(optimizer=optimizer, lr=lr, steps_per_epoch=steps_per_epoch, num_epochs=num_epochs, scheduler_epochs=scheduler_epochs, resume_step=resume_step)

    if scheduler_path is not None and scheduler_path.exists():
    ...
    for epoch in range(start_epoch, num_epochs):
        train_loss, train_acc, time_per_epoch = train_one_epoch(model=model, trainset=dataset_dict["trainset"], batch_size=batch_size, optimizer=optimizer, scheduler=scheduler, criterion=criterion, device=device, num_workers=num_workers, grad_clip=grad_clip, step_scheduler_per_batch=True)
        ...
        if should_eval:
            ...
            if val_acc > best_val_acc:
                ...
                best_test_at_best_val_acc = test_acc
    ...
    if save_outputs:
        params = {"seed": seed, "num_epochs": num_epochs, "batch_size": batch_size, "lr": lr, "momentum": momentum, "weight_decay": weight_decay, "width_mult": width_mult, "grad_clip": grad_clip, "best_val_acc": best_val_acc, "best_test_acc": best_test_acc, 
                  "best_test_at_best_val_acc": best_test_at_best_val_acc, "final_test_acc": final_test_acc, "late_window_best_test_acc": late_window_best_test_acc, "late_window_gap": late_window_gap, "model::type": "ResNet18Slim", "model::channels_in": int(dataset_meta["channels_in"]), 
                  "model::o_dim": int(dataset_meta["num_classes"]), "model::nlin": nlin, "model::dropout": dropout, "model::init_type": init_type, "model::width_mult": width_mult}
...
def get_completed_seeds(experiment_dir: Path, expected_epochs: int, expected_lr: float | None = None) -> set[int]:
    ...
    for seed_dir in experiment_dir.glob("NN_tune_trainable_seed=*"):
        ...
        if int(params.get("num_epochs", 0)) < int(expected_epochs):
            continue
        if expected_lr is not None and not math.isclose(float(params.get("lr", float("nan"))), float(expected_lr), rel_tol=0.0, abs_tol=1e-12):
            continue
...
def task_label(task: dict[str, Any]) -> str:
    ...
    return f"{mode}:seed={task['seed']}:lr={format_lr_value(float(task['lr']))}"
...
def worker_main(worker_id: int, device: str, dataset_pt: str, output_root: str, shared_config: dict[str, Any], tasks: list[dict[str, Any]], result_queue: Any) -> None:
    ...
    for task in tasks:
        try:
            summary = train_single_model(seed=int(task["seed"]), dataset_dict=dataset_dict, dataset_meta=dataset_meta, output_root=Path(output_root), device=device, num_epochs=int(shared_config["epochs"]), scheduler_epochs=shared_config["scheduler_epochs"], checkpoint_start_epoch=int(shared_config["checkpoint_start_epoch"]), 
                                         eval_every=int(shared_config["eval_every"]), batch_size=int(shared_config["batch_size"]), lr=float(task.get("lr", shared_config["lr"])), momentum=float(shared_config["momentum"]), weight_decay=float(shared_config["weight_decay"]), width_mult=float(shared_config["width_mult"]), 
                                         num_workers=int(shared_config["num_workers"]), nlin=str(shared_config["nlin"]), dropout=float(shared_config["dropout"]), init_type=str(shared_config["init_type"]), grad_clip=float(shared_config["grad_clip"]), save_outputs=bool(task["save_outputs"]))
...
def build_sweep_candidate(lr: float, runs: list[dict[str, Any]]) -> dict[str, Any]:
    if not runs:
        return {"lr": float(lr), "lr_label": format_lr_value(float(lr)), "successful_runs": 0, "mean_late_window_best_test_acc": float("-inf"), "mean_late_window_gap": float("inf"), "mean_final_test_acc": float("-inf"), "median_final_test_acc": float("-inf"), "min_final_test_acc": float("-inf"), "std_final_test_acc": float("inf"), "robust_final_test_acc": float("-inf"), "runs": []}
    ...
    return {"lr": float(lr), "lr_label": format_lr_value(float(lr)), "successful_runs": len(runs), "mean_late_window_best_test_acc": mean_late_window_best, "mean_late_window_gap": mean_late_window_gap, "mean_final_test_acc": mean_final, "median_final_test_acc": median_final, "min_final_test_acc": min_final, "std_final_test_acc": std_final, "robust_final_test_acc": robust_final, "runs": runs}
...
def build_sweep_summary(dataset_name: str, candidates: list[dict[str, Any]]) -> dict[str, Any]:
    chosen = choose_sweep_candidate(candidates)
    return {"dataset": dataset_name, "chosen_lr": chosen["lr"], "chosen_lr_label": chosen["lr_label"], "selection_strategy": LR_SELECTION_STRATEGY, "candidates": candidates}


def maybe_write_sweep_summary(summary: dict[str, Any], summary_path: Path, persist_summary: bool) -> dict[str, Any]:
...
def summarize_sweep_results(dataset_name: str, results: list[dict[str, Any]], lr_candidates: list[float], summary_path: Path, persist_summary: bool) -> dict[str, Any]:
    ...
    return maybe_write_sweep_summary(build_sweep_summary(dataset_name=dataset_name, candidates=candidates), summary_path=summary_path, persist_summary=persist_summary)


def normalize_existing_sweep_summary(dataset_name: str, summary_payload: dict[str, Any], summary_path: Path, persist_summary: bool) -> dict[str, Any]:
...
def load_or_run_lr_sweep(dataset_name: str, dataset_pt: Path, output_root: Path, lr_candidates: list[float], lr_sweep_seeds: int, force_lr_sweep: bool, shared_config: dict[str, Any], devices: list[str], models_per_gpu: int, persist_summary: bool) -> dict[str, Any]:
    summary_path = output_root / "lr_sweep" / "summary.json"
    if persist_summary and summary_path.exists() and not force_lr_sweep:
        with summary_path.open() as handle:
            return normalize_existing_sweep_summary(dataset_name=dataset_name, summary_payload=json.load(handle), summary_path=summary_path, persist_summary=persist_summary)
    ...
    return summarize_sweep_results(dataset_name=dataset_name, results=results, lr_candidates=lr_candidates, summary_path=summary_path, persist_summary=persist_summary)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lr-sweep-seeds", type=int, default=4, help="Number of sweep seeds per LR candidate")
    parser.add_argument("--force-lr-sweep", action="store_true", help="Rerun the LR sweep even if a saved summary exists")
    parser.add_argument("--no-lr-sweep", action="store_true", help="Skip LR sweep and train final seeds directly with --lr")
    parser.add_argument("--epochs", type=int, default=25, help="Number of training epochs")
    parser.add_argument("--scheduler-epochs", type=int, default=50, help="OneCycleLR schedule length in epochs")
    ...
    parser.add_argument("--lr", type=float, default=0.05, help="Max learning rate for SGD + OneCycleLR")
    parser.add_argument("--momentum", type=float, default=0.9, help="Momentum for SGD")
    parser.add_argument("--weight-decay", type=float, default=5e-4, help="Weight decay")
    parser.add_argument("--width-mult", type=float, default=0.5, help="ResNet18Slim width multiplier")
    parser.add_argument("--nlin", default="relu", help="ResNet18Slim nonlinearity")
    parser.add_argument("--dropout", type=float, default=0.2, help="ResNet18Slim dropout")
    parser.add_argument("--init-type", choices=["uniform", "normal"], default="uniform", help="ResNet18Slim init type")
    parser.add_argument("--models-per-gpu", type=int, default=1, help="Parallel models to run on each GPU")
    ...
    parser.add_argument("--grad-clip", type=float, default=1.0, help="Max gradient norm, or 0 to disable")
...
def main() -> None:
    ...
    output_root = resolve_output_root(args.dataset, args.zoo_root, args.width_mult)
    if not args.dry_run:
    ...
    print(f"Model config: channels_in={dataset_meta['channels_in']} num_classes={dataset_meta['num_classes']} width_mult={args.width_mult} nlin={args.nlin} dropout={args.dropout} init_type={args.init_type}")
    print(f"Training config: epochs={args.epochs} scheduler_epochs={args.scheduler_epochs} batch_size={args.batch_size} lr={args.lr} momentum={args.momentum} weight_decay={args.weight_decay} grad_clip={args.grad_clip}")
    if args.no_lr_sweep:
        print("LR sweep: disabled")
    else:
        print(f"LR sweep: seeds={args.lr_sweep_seeds} candidates={[float(value) for value in args.lr_candidates]}")

    devices = available_devices(args.gpu_id)
    print(f"Devices: {devices}")
    shared_config = {"epochs": int(args.epochs), "scheduler_epochs": int(args.scheduler_epochs), "checkpoint_start_epoch": int(args.checkpoint_start_epoch), "eval_every": int(args.eval_every), "batch_size": int(args.batch_size), "lr": float(args.lr), "momentum": float(args.momentum), 
                     "weight_decay": float(args.weight_decay), "width_mult": float(args.width_mult), "num_workers": int(args.num_workers), "nlin": str(args.nlin), "dropout": float(args.dropout), "init_type": str(args.init_type), "grad_clip": float(args.grad_clip)}

    chosen_lr = float(args.lr)
    if args.no_lr_sweep:
        print(f"Chosen LR: fixed ({chosen_lr})")
    else:
        sweep_summary = load_or_run_lr_sweep(dataset_name=args.dataset, dataset_pt=source_dataset_pt, output_root=output_root, lr_candidates=[float(value) for value in args.lr_candidates], lr_sweep_seeds=int(args.lr_sweep_seeds), 
                                             force_lr_sweep=bool(args.force_lr_sweep), shared_config=shared_config, devices=devices, models_per_gpu=int(args.models_per_gpu), persist_summary=not args.dry_run)
        chosen_lr = float(sweep_summary["chosen_lr"])
        print(f"Chosen LR: {sweep_summary['chosen_lr_label']} ({chosen_lr})")

    completed = set() if args.dry_run else get_completed_seeds(output_root, expected_epochs=args.epochs, expected_lr=chosen_lr)
    if not args.dry_run:
